...
async function uploadS() {
  ...
  for (let i = 0; i < stations.length; i++) {
    ...
    await db("se_project.stations").insert({
      stationname: element.stationName,
      stationtype: element.stationType,
      stationposition: element.stationPosition,
      stationstatus: element.stationStatus
    }).returning("*");
  }
}
async function uploadR() {
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    await db("se_project.routes").insert({
      fromstationid: element.fromStationId,
      routename: element.routeName,
      tostationid: element.toStationId
    }).returning("*");
  }
}
uploadS();
uploadR();
uploadSR();
db.select('*').from('se_project.stationroutes')
  .then((rows) => {
    console.log(rows);
  })
  .catch((error) => {
